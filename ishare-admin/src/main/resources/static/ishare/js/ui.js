...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            // 回显用户信息
            selectDictUser: function(datas, value) {
            	var actions = [];
            	$.each(datas, function(index, dict) {
            		console.log(dict.loginName+' > '+value);
                    if (dict.loginName == value) {
                    	actions.push(dict.userName);
                        return false;
                    }
                });
                return actions.join('');
            ...
            // 回显部门信息
            selectDictDept: function(datas, value) {
            	var actions = [];
            	$.each(datas, function(index, dict) {
                    if (dict.deptId == value) {
                    	actions.push(dict.deptName);
                        return false;
                    }
                });
                return actions.join('');
        ...
        operate: {
            ...
            // 修改信息
            editByWindow: function(id,width,height) {
            	var url = "/404.html";
            	if ($.common.isNotEmpty(id)) {
            	    url = $.table._option.updateUrl.replace("{id}", id);
            	} else {
