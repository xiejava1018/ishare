...
$(function() {
	...
	$("#bootstrap-table")
			.on(
					...
					function() {
						var ids = $("#bootstrap-table").bootstrapTable(
								"getSelections");
						$('#toolbar .btn-del').toggleClass('disabled',
								!ids.length);
						$('#toolbar .btn-edit').toggleClass('disabled',
								ids.length != 1);
						;
					});
	// tree表格树 展开/折叠
...
var refreshItem = function() {
	...
	var target = $('.Ishare_iframe[data-id="' + currentId + '"]', topWindow);
...
function createMenuItem(dataUrl, menuName) {
			...
			function() {
				if ($(this).data('id') == dataUrl) {
					if (!$(this).hasClass('active')) {
						...
						// 显示tab对应的内容区
						$('.mainContent .Ishare_iframe', topWindow).each(
								function() {
									if ($(this).data('id') == dataUrl) {
										$(this).show().siblings(
												'.Ishare_iframe').hide();
	...
	if (flag) {
		...
		// 添加选项卡对应的iframe
		var str1 = '<iframe class="Ishare_iframe" name="iframe' + dataIndex
				+ '" width="100%" height="100%" src="' + dataUrl
				+ '" frameborder="0" data-id="' + dataUrl
				+ '" seamless></iframe>';
		$('.mainContent', topWindow).find('iframe.Ishare_iframe').hide()
